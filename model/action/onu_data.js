...
function getOverAllNEData(data) {
  let sql =
    "SELECT NE_Name,count(*) AS ne_count FROM aarx_status WHERE status = 1 " +
    "AND NE_name REGEXP '" +
    data +
...
function getDataByPrefix(prefix) {
  let sql =
    "SELECT count(*) AS ne_count FROM aarx_status WHERE status = 1 AND NE_Name LIKE '" +
    prefix +
    "%'";
  //console.log(sql);
  return new Promise(function (resolve, reject) {
...
function get_PON_data_by_prefix(prefix) {
  let sql =
    "SELECT NE_Name,count(*) AS ne_count FROM aarx_status WHERE status = 1 AND NE_Name LIKE '" +
    prefix +
    "%' GROUP BY NE_Name";
  //console.log(sql);
  return new Promise(function (resolve, reject) {
...
function getMasterIDByPrefix(prefix) {
  let sql =
    "SELECT master_id,count(*) AS master_count FROM aarx_status WHERE status = 1 AND NE_Name LIKE '" +
    prefix +
    "%' GROUP BY master_id ORDER BY create_at DESC";
  //console.log(sql);
  return new Promise(function (resolve, reject) {
    db_conn.query(sql, function (err, rows, fields) {
      if (err) throw err;
...
async function getActiveMasterIDByPrefix(prefix) {
  ...
  let sql_1 =
    "SELECT * FROM aarx_master WHERE status = 1 ORDER BY created_at DESC";
  let sql_2 =
    "SELECT NE_Name,count(*) AS pon_count FROM aarx_status WHERE status = 1 AND NE_Name LIKE '" +
    prefix +
    "%' GROUP BY NE_Name";
  //console.log(sql);
...
function countPONByMasterID(master_id, prefix) {
  let sql =
    "SELECT master_id,count(*) AS pon_count FROM import_data WHERE NE_Name LIKE '" +
    prefix +
    "%' AND master_id = " +
    master_id +
    " GROUP BY master_id";
...
function getTXONUCount(data) {
  return new Promise(function (resolve, reject) {
    let sql =
      "SELECT DATE_FORMAT(create_at, '%Y-%m-%d') AS dates FROM aarx_status WHERE NE_Name LIKE '" +
      data.prefix +
    ...
    db_conn.query(sql, function (err, rows, fields) {
      ...
      // get all master id that use by those prefix
      let sql_1 =
        "SELECT * FROM import_data WHERE NE_Name LIKE '" +
        data.prefix +
        "%' AND start_at LIKE '" +
        curr_date +
        "%'";
      ...
      db_conn.query(sql_1, function (err, _rows, fields) {
        ...
        onu_count.bad = bad;
        console.log(
          "onu TX count result for " +
            data.prefix +
            " " +
            JSON.stringify(onu_count)
        );
        resolve(onu_count);
      });
    });
  ...
  return new Promise(function (resolve, reject) {
    ...
    db_conn.query(sql, function (err, rows, fields) {
      ...
      let sql_1 =
        ...
        "%' AND start_at LIKE '" +
        curr_date +
        "%' AND Transmitted_Optical_Power < 2 AND NOT Transmitted_Optical_Power = '--'";
      //console.log(sql_1);
      db_conn.query(sql_1, function (err, _rows, fields) {
        if (err) throw err;
        resolve(_rows);
...
function getRXONUCount(data) {
  return new Promise(function (resolve, reject) {
    ...
    db_conn.query(sql_0, async function (err, rows, fields) {
      ...
      // get all master id that use by those prefix
      let sql_1 =
        "SELECT * FROM aarx_status WHERE NE_Name LIKE '" +
        data.prefix +
        "%' AND status = 1";
      // get all master id that use by those prefix
      let sql_2 =
        "SELECT * FROM import_data WHERE start_at LIKE '" +
        curr_date +
        "%' AND NE_Name LIKE '" +
        data.prefix +
        "%'";
      ...
      return Promise.all([pons, onus]).then((res) => {
        ...
        onu_count.bad = bad;
        console.log(
          "onu RX result for " + data.prefix + " " + JSON.stringify(onu_count)
        );
        resolve(onu_count);
      });
    });
...
async function _getRXONUData(data) {
  ...
  let sql_1 =
    "SELECT * FROM aarx_status WHERE status = 1 AND master_id = " +
    data.master_id +
    " AND NE_Name LIKE '" +
    data.prefix +
    "%'";
  // get all master id that use by those prefix
  let sql_2 =
    "SELECT * FROM import_data WHERE master_id = " +
    data.master_id +
    " AND NE_Name LIKE '" +
    data.prefix +
    "%'";
...
async function getCountNCONUData(data) {
  ...
  return Promise.all([data_date]).then((dates) => {
    ...
    let sql_current =
      "SELECT * FROM aarx_status WHERE create_at like '" +
      curr_date +
      "%' AND NE_Name LIKE '" +
      data.prefix +
      "%'";
