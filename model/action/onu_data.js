...
function getOverAllNEData(data) {
  let sql = "SELECT NE_Name,count(*) AS ne_count FROM aarx_status WHERE status = 1 " + "AND NE_name REGEXP '" + data + "' GROUP BY substr(NE_Name, 1,3)";
...
function countPONByMasterID(master_id, prefix) {
  let sql = "SELECT master_id,count(*) AS pon_count FROM import_data WHERE NE_Name LIKE '" + prefix + "%' AND master_id = " + master_id + " GROUP BY master_id";
  //console.log(sql);
  return new Promise(function (resolve, reject) {
    db_conn.query(sql, function (err, rows, fields) {
      if (err) throw err;
...
function getTXONUCount(data) {
  return new Promise(function (resolve, reject) {
    ...
    db_conn.query(sql, function (err, rows, fields) {
      ...
      // get all master id that use by those prefix
      let sql_1 = "SELECT * FROM import_data WHERE NE_Name LIKE '" + data.prefix + "%' AND start_at LIKE '" + curr_date + "%'";
      //console.log(sql_1);
...
function get_TX_NC_ONU_data(data) {
  return new Promise(function (resolve, reject) {
    ...
    db_conn.query(sql, function (err, rows, fields) {
      ...
      let curr_date = rows[0].dates;
      // get all master id that use by those prefix
      let sql_1 = "SELECT * FROM import_data WHERE NE_Name LIKE '" + data.prefix + "%' AND start_at LIKE '" + curr_date + "%' AND Transmitted_Optical_Power < 2 AND NOT Transmitted_Optical_Power = '--'";
      //console.log(sql_1);
      db_conn.query(sql_1, function (err, _rows, fields) {
        if (err) throw err;
        resolve(_rows);
  ...
  return new Promise(function (resolve, reject) {
    ...
    db_conn.query(
      ...
      async function getPONONURXData(data) {
        // get all master id that use by those prefix
        let sql = "SELECT * FROM import_data WHERE NE_Name LIKE '" + data.ne_name + "' AND Rack = " + data.rack + " AND Shelf = " + data.shelf + " AND Slot = " + data.slot + " AND Port = " + data.port + " AND (master_id = -1";
...
async function getCountNCONUData(data) {
  ...
  return Promise.all([data_date]).then((dates) => {
    ...
    let sql_current = "SELECT * FROM aarx_status WHERE create_at like '" + curr_date + "%' AND NE_Name LIKE '" + data.prefix + "%'";
    let sql_previous = "SELECT * FROM aarx_status WHERE create_at like '" + prev_date + "%' AND NE_Name LIKE '" + data.prefix + "%'";
    // get all master id that use by those prefix
    let sql_2 = "SELECT * FROM import_data WHERE NE_Name LIKE '" + data.prefix + "%' AND start_at LIKE '" + curr_date + "%'";
